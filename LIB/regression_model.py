...
class TFBaseModel(object):
    ...
    def fit(self):
        ...
        self.restart_model()
        self.prepare_history_paramters()

        while self.epoch < self.num_epoch:
            mean_train, mean_val=self.train_epoch()
            self.log_epoch(mean_train, mean_val)

            if (self.save_with_best_score_down(mean_val)): return
    ...
    def save_with_best_score_down(self, new_score):
        if self.epoch >= self.min_epoch_to_checkpoint:
            if new_score < self.best_validation_score_down:
                ...
                self.best_validation_epoch = self.epoch

                logging.info('best validation score  of {} at test step {}'.format(
                    self.best_validation_score_down, self.best_validation_epoch))
                self.save(self.epoch)
                if self.enable_parameter_averaging:
                    self.save(self.epoch, averaged=True)

        if self.epoch - self.best_validation_epoch > self.early_stopping_epochs:
            ...
            if self.restarts < self.num_restarts:
                ...
                self.epoch = self.best_validation_epoch
                self.restarts += 1
    ...
    def prepare_history_paramters(self):
        self.train_generator = self.reader.train_epoch_generator(self.num_epoch*self.num_restarts)
        self.validation_generator = self.reader.val_epoch_generator(self.num_epoch**self.num_restarts)
        self.train_loss_history = []
        self.validation_loss_history = []
        self.best_validation_score_down, self.best_validation_epoch = float('inf'), 0
        self.best_validation_score, self.best_validation_epoch = float('-inf'), 0
        self.restarts = 0
        self.step_train = 0
    def train_epoch(self):
        self.step_display = 0

        self.train_batches = next(self.train_generator)
        self.validation_batches = next(self.validation_generator)

        self.train_loss_epoch = []
        self.validation_loss_epoch = []
        # train
        for i, train_batch_df in enumerate(self.train_batches.batch_generator(self.batch_size)):
            try:
                validation_batch_df = next(self.validation_batches)
            except Exception as ex:
                validation_batche = self.validation_batches.batch_generator(self.num_validation_batches * self.batch_size)
                validation_batch_df = next(validation_batche)

            self.train_one_step(train_batch_df,validation_batch_df)
        # val
        for i, val_batch_df in enumerate(self.validation_batches.batch_generator(self.num_validation_batches * self.batch_size)):
            validation_loss, _ =self.val_one_step(val_batch_df)
            self.validation_loss_epoch.append(validation_loss)
        mean_train=np.mean(self.train_loss_epoch)
        mean_val=np.mean(self.validation_loss_epoch)

        self.train_loss_history.append(mean_train)
        self.validation_loss_history.append(mean_val)
        return mean_train ,mean_val

    def train_one_step(self,train_batch_df,validation_batches):
        ### train
        train_feed_dict = {
            getattr(self, placeholder_name, None): data
        ...
        train_loss, grad, summary_train= self.session.run(
            fetches=[self.loss, self.step, self.merge_Sum],
            feed_dict=train_feed_dict
        )
        ### val
        val_loss , summary_val=self.val_one_step(validation_batches)
        self.step_train += 1

        self.train_writer.add_summary(summary_train, self.step_train)
        self.test_writer.add_summary(summary_val, self.step_train)

        self.validation_loss_epoch.append(val_loss)
        self.train_loss_epoch.append(train_loss)
        ### log
        if self.step_display == self.display_step:
            metric_log = (
                "[[step {:>8}]]   "
                "[[train]]   loss: {:<8}  -ame :{:<8}"

            ).format(self.step_train, round(np.mean(self.train_loss_epoch), 5),
                     round(np.mean(self.validation_loss_epoch), 5))
            stdout.write("\r%s" % metric_log)
            self.step_display = 0
        self.step_display+=1

    def val_one_step(self, validation_batches):

        validation_feed_dict = {
            getattr(self, placeholder_name, None): data
            for placeholder_name, data in validation_batches if hasattr(self, placeholder_name)
        }
        if hasattr(self, 'keep_prob'):
        ...
        validation_loss, summary = self.session.run(
            fetches=[self.loss, self.merge_Sum],
            feed_dict=validation_feed_dict
        )
        return validation_loss,summary
    def log_epoch(self,mean_train,mean_val):
        # stdout.write("\n")

        metric_log = (
            "[[epoch {:>8}]]     "
            "[[train]]     loss: {:<12}--[[val]]     loss: {:<12}"

        ).format(self.epoch, round(mean_train, 8), round(mean_val, 8)
                 )
