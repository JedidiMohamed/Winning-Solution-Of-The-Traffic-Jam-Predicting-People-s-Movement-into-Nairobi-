...
class TFBaseModel(object):
    ...
    def fit(self):
        ...
        with self.session.as_default():
            ...
            validation_generator = self.reader.val_epoch_generator(self.num_epoch*5)
            ...
            ############################Train#####################################
                ...
                for i, train_batch_df in enumerate(train_batches.batch_generator(self.batch_size)):
                    ...
                    train_loss, grad,train_accuracy,summary= self.session.run(
                        fetches=[self.loss, self.step,self.accuracy, self.merge_Sum],
                        feed_dict=train_feed_dict
                    )

                    self.train_writer.add_summary(summary, self.train_steps)
                    train_loss_epoch.append(train_loss)
                    train_accuracy_epoch.append(train_accuracy)
                    try:

                        validation_batch_df=next(validation_batches)
                    except Exception as ex: 
                        validation_batche=validation_batches.batch_generator(self.num_validation_batches*self.batch_size)
                        validation_batch_df=next(validation_batche)
                    validation_loss,validation_accuracy =self.Validation(validation_batch_df)
                    if step_display == 300 :

                        metric_log = (
                                        "[[step {:>8}]]   "
                                        "[[train]]   loss: {:<8}  -accuracy :{:<8} -"
                                        "[[Vali]]   loss: {:<8}  -accuracy :{:<8}"



                                    ).format(self.epoch, round(train_loss, 8),round(train_accuracy, 8),
                                    round(validation_loss, 8),round(validation_accuracy, 8))
            ...
            ############################Validation#####################################
                ...
                for i, validation_batch_df in enumerate(validation_batches.batch_generator(self.num_validation_batches*self.batch_size)):
                    ...
                    target,predictions,validation_accuracy,validation_loss= self.session.run(
                    fetches=[self.target,self.predictions,self.accuracy,self.loss],
            ...
            ######################## DISPLAY ##########################################################
                ...
                metric_log = (
                                        "[[epoch {:>8}]]     "
                                        "[[train]]     loss: {:<12}-accuracy :{:<12} \n"
                                        "[[val]]       loss: {:<12}-accuracy :{:<12} "
                ...
                if self.log_metrics :
                    ...
                    logging.info("log_metrics Val :"+str(score))
                    if self.Max :
                        if(self.save_with_best_score_up(score)): break
                    else :
                        if(self.save_with_best_score_down(validation_mean_loss)):break
                else : 

                    if self.Max :
                        score=validation_mean_accuracy
                        if(self.save_with_best_score_up(score)): break
                    else :
                        score=validation_mean_loss
                        if(self.save_with_best_score_down(validation_mean_loss)):break
...
#         logging.info(str(np.sum(np.prod(shape(var)) for var in tf.trainable_variables())))
    ...
    def tensorbord(self,graph):
        with tf.name_scope('Tensorboard'):
            if not os.path.isdir(self.Tensorbord_dir):
    ...
    def save_with_best_score_up(self,new_score):
        if new_score > self.best_validation_score:


            if self.epoch >= self.min_epoch_to_checkpoint:
                self.best_validation_score = new_score
                self.best_validation_tstep = self.epoch
                logging.info('best validation score  of {} at test step {}'.format(
                    self.best_validation_score, self.best_validation_tstep))
                self.save(self.epoch)
                if self.enable_parameter_averaging:
                    self.save(step, averaged=True)
        ...
        if self.epoch - self.best_validation_tstep > self.early_stopping_epochs:
            ...
            if self.restarts < self.num_restarts:
                ...
                #self.early_stopping_epochs /= 2.0
    ...
    def save_with_best_score_down(self,new_score):
        ...
        if self.epoch - self.best_validation_tstep > self.early_stopping_epochs:
            ...
            if self.restarts < self.num_restarts:
                ...
                self.restarts += 1
        return False
    def Validation(self,validation_batch_df):
        ...
        target,predictions,validation_accuracy,validation_loss,summary= self.session.run(
        fetches=[self.target,self.predictions,self.accuracy,self.loss, self.merge_Sum],
                  feed_dict=validation_feed_dict)
        self.test_writer.add_summary(summary, self.train_steps)
